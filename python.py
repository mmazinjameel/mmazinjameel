...
# SVG files
DARK_MODE_SVG = "dark_mode.svg"
LIGHT_MODE_SVG = "light_mode.svg"

def fetch_github_stats(username):
    try:
        # Fetch number of public repositories
        ...
        # Fetch number of commits (approximation)
        num_commits = 0
...
def update_svg_file(svg_file, num_repos, num_commits, total_loc, uptime):
    ...
    svg_content = re.sub(r'<tspan class="value" id="contrib_data">.*?</tspan>', f'<tspan class="value" id="contrib_data">0</tspan>', svg_content)  # Assuming no contributed repos
...
def main():
    ...
    update_svg_file(DARK_MODE_SVG, num_repos, num_commits, total_loc, uptime)
    update_svg_file(LIGHT_MODE_SVG, num_repos, num_commits, total_loc, uptime)
